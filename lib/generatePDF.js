...
const normVerdict = v => (LEGACY_VERDICTS[v] || v);

function verdictColor(v) {
  v = normVerdict(v);
  if (v === 'PRESENT') return C.green;
  if (v === 'PRESENT_WITH_FLAGS') return C.yellow;
  if (v === 'NOT_WORTH_IT') return C.orange;
  return C.red;
}
...
function verdictBg(v) {
  ...
  if (v === 'NOT_WORTH_IT') return [234, 88, 12];
  return [220, 38, 38];
}

function verdictLabel(v) {
  v = normVerdict(v);
  if (v === 'PRESENT') return 'HELL YEAH!';
  if (v === 'PRESENT_WITH_FLAGS') return 'YES, BUT';
  if (v === 'NOT_WORTH_IT') return 'PROBABLY NOT';
  return 'NOPE!';
}
...
function drawReport(doc, result, context, PAGE_W, MARGIN, CONTENT_W, bodyX, bodyW, measureOnly) {
  ...
  // ── Scope note (engagement-level, applies to all candidates) ──
  if (result.scope_note) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6.5);
    const scopeLines = wrapText(doc, result.scope_note, bodyW - 24);
    const scopeH = scopeLines.length * 3.6 + 7;

    if (!measureOnly) {
      rr(doc, bodyX, y, bodyW, scopeH, 2, [42, 31, 8], [92, 69, 20]);
      doc.setFont('courier', 'bold');
      doc.setFontSize(4.5);
      doc.setTextColor(...C.yellow);
      doc.text('SCOPE', bodyX + 4, y + 5);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(6.5);
      doc.setTextColor(230, 213, 168);
      scopeLines.forEach((line, i) => doc.text(line, bodyX + 18, y + 5 + i * 3.6));
    }
    y += scopeH + 5;
  }
  ...
  if (result.scores) {
    for (const key of Object.keys(CORE_WEIGHTS)) {
      ...
      const barX = bodyX + 55;
      const barW = 32;
      const labelX = barX + barW + 10;

      // Measure the label first so the row can grow rather than truncating it.
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(5.5);
      const labelLines = wrapText(doc, val.label || '', bodyW - (labelX - bodyX) - 2);
      const rowH = Math.max(9, labelLines.length * 3.2 + 5.5);
      ...
      if (!measureOnly) {
        ...
        doc.setFontSize(5.5);
        doc.setTextColor(...C.textSecondary);
        labelLines.forEach((line, i) => doc.text(line, labelX, y + 3 + i * 3.2));
      }
      y += rowH;
    }
  }
  ...
  if (bonusEntries.length > 0) {
    ...
    for (const key of bonusEntries) {
      const meta = DIM_LABELS[key];
      const val = result.scores[key];
      const labelX = bodyX + 82;

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(5.5);
      const labelLines = wrapText(doc, val.label || '', bodyW - (labelX - bodyX) - 4);
      const rowH = Math.max(8, labelLines.length * 3.2 + 4.5);

      if (!measureOnly) {
        const color = scoreColor(val.score);
        rr(doc, bodyX, y - 3.5, bodyW, rowH, 1.5, C.analysisBg, null);
        ...
        doc.setFontSize(5.5);
        doc.setTextColor(...C.textSecondary);
        labelLines.forEach((line, i) => doc.text(line, labelX, y + 1.5 + i * 3.2));
      }
      y += rowH + 0.5;
    }
  }
