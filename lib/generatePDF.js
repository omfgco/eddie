...
function buildReportHTML(result, ctx) {
  ...
  let suggestionsHTML = '';
  for (const s of (result.ownability_suggestions || [])) {
    suggestionsHTML += `
      <div style="padding:10px 14px;background:#0d2818;border-radius:6px;margin-bottom:6px;border-left:3px solid #22c55e">
        <div style="font-weight:700;font-size:14px;color:#e6edf3">${s.suggestion}</div>
        <div style="font-size:13px;color:#8b949e;margin-top:3px;line-height:1.5">${s.rationale}</div>
      </div>`;
  }
      ...
      <div style="max-width:900px;margin:0 auto;padding:24px 28px 40px">
        ...
        <div style="background:#131920;border:1px solid #242d38;border-radius:10px;overflow:hidden;margin-bottom:16px">
          ...
          <div style="padding:16px 20px 20px">
            ...
            <div>
              <h4 style="font-family:monospace;font-size:10px;font-weight:800;letter-spacing:1.8px;text-transform:uppercase;color:#8b949e;margin:0 0 12px">How to Make It More Ownable</h4>
              ${suggestionsHTML}
            </div>` : ''}
          </div>
        </div>
